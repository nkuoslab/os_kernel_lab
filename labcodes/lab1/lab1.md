...
- `make`是一个工具程序，通过读取`makefile`文件，自动化建构软件。它是一种转化文件形式的工具，与此同时也检查文件的依赖关系。在这个过程中，他会调用一些外部软件来完成任务。简单的说，`make`是一个根据指定的`shell`命令进行构建的工具。

- `makefile`由一系列规则构成，每条规则的结构如下：
  ...
  - 第一行冒号前是目标，冒号后是前置条件；第二行必须以`tab`开始，后面是命令。

    其中目标是必须的，前置条件和命令是可选的，但二者不可均缺失。

  - 目标通常是文件名，可以是多个，用空格分割。也可以是某个操作的名字，称为伪目标。

    为避免重名现象，可以用`.PHONY:<伪命令> `具体声明伪目标。

  - 前置条件通常是一组文件名，之间用空格分隔。它指定了目标是否重新构建的判断标准：只要有一个前置文件不存在，或者有过更新，目标就需要重新构建。

  - 命令表示如何更新目标文件，由一行或多行的`shell`命令组成。它是构建目标的具体指令，它的运行结果通常就是生成目标文件。

    - 每行命令在一个单独的`shell`中执行。这些`shell`之间没有继承关系。可以通过在一行末尾加入`\`转义，也可以在目标前加上`.ONESHELL`命令。
...
- `makefile`文件语法：
  ...
  - `make`会打印出每条命令后再执行。在命令前加上`@`便可以不再打印该条命令。

  - 通配符：与`bash`语法一致。

  - 模式匹配：利用匹配符`%`可以批量处理文件。如`%.o : %.c`可以一次性编译所有的c文件。

  - 利用等号可以给变量赋值，调用时，变量需要放在`$()`之中。调用`shell`变量时，需要在美元符号前再加一个美元符号如`echo $$HOME$`。

  - `make`中的赋值运算符还有四种，具体如下：
