...
static struct proc_struct* alloc_proc(void) {
    ...
    if (proc != NULL) {
         ...
         * times of Proces uintptr_t kstack;                           //
         * Process kernel stack volatile bool need_resched;                 //
         * bool value: need to be rescheduled to release CPU? struct proc_struct
         * *parent;                 // the parent process struct mm_struct *mm;
         * // Process's memory management field struct context context; //
         * Switch here to run process struct trapframe *tf; // Trap frame for
         * current interrupt uintptr_t cr3;                              // CR3
         * register: the base addr of Page Directroy Table(PDT) uint32_t flags;
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe* tf) {
     ...
     * implementation. MACROs or Functions: alloc_proc:   create a proc struct
     * and init fields (lab4:exercise1) setup_kstack: alloc pages with size
     * KSTACKPAGE as process kernel stack copy_mm:      process "proc" duplicate
     * OR share process "current"'s mm according clone_flags if clone_flags &
     * CLONE_VM, then "share" ; else "duplicate" copy_thread:  setup the
     * trapframe on the  process's kernel stack top and setup the kernel entry
     * point and stack of process hash_proc:    add proc into proc hash_list
     *   get_pid:      alloc a unique pid for process
...
static int load_icode(unsigned char* binary, size_t size) {
     ...
     * the top addr of user stack (USTACKTOP) tf_eip should be the entry point
     * of this binary program (elf->e_entry) tf_eflags should be set to enable
