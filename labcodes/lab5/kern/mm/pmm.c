...
void pmm_init(void) {
    ...
    // we should reload gdt (second time, the last time) to get user segments
    // and the TSS map virtual_addr 0 ~ 4G = linear_addr 0 ~ 4G then set kernel
...
pte_t* get_pte(pde_t* pgdir, uintptr_t la, bool create) {
     ...
     * kernel virtual address. set_page_ref(page,1) : means the page be
     * referenced by one time page2pa(page): get the physical address of memory
     * which this (struct Page *) page  manages struct Page * alloc_page() :
     * allocation a page memset(void *s, char c, size_t n) : sets the first n
     * bytes of the memory area pointed by s to the specified value c. DEFINEs:
     *   PTE_P           0x001                   // page table/directory entry
     * flags bit : Present PTE_W           0x002                   // page
     * table/directory entry flags bit : Writeable PTE_U           0x004 // page
     * table/directory entry flags bit : User can access
     */
#if 0
...
static inline void page_remove_pte(pde_t* pgdir, uintptr_t la, pte_t* ptep) {
     ...
     * value of a ptep free_page : free a page page_ref_dec(page) : decrease
     * page->ref. NOTICE: ff page->ref == 0 , then this page should be free.
     *   tlb_invalidate(pde_t *pgdir, uintptr_t la) : Invalidate a TLB entry,
     * but only if the page tables being edited are the ones currently in use by
     * the processor. DEFINEs: PTE_P           0x001                   // page
...
int copy_range(pde_t* to,
    ...
    do {
        ...
        if (*ptep & PTE_P) {
             ...
             * pmm.h) page_insert: build the map of phy addr of an Page with the
             * linear addr la memcpy: typical memory copy function
             *
